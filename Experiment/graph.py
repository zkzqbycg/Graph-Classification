...
def load_edgelist(undirected=True):
  G = Graph()
  with open('graph/barbell.edgelist') as f:
    for l in f:
...
def from_pyg(data, undirected=True):
    G = Graph()

    for i in range(data.num_nodes):
        G[i] = []

    # 将每条边的两个节点添加到相应的邻居列表中
    for j in range(data.edge_index.size(1)):
        source, target = data.edge_index[0][j].item(), data.edge_index[1][j].item()
        if target not in G[source]:
            G[source].append(target)
        if source not in G[target]:
